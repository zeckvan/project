<template>
  <div>
    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="學習成果及多元表現概況統計" name="first">
          <TeaTutorResult 
...
<script>
  var apiurl = ''
  import TeaTutorResult from '@/components/teacher/tea_tutor_result.vue'
      ...
      _self.$http({
        ...
        .then((res) => {  
          if (res.data.status == 'Y') {     
            ...
            _self.page2Lable = `學習成果及多元表現概況明細(學號：)`
          }
        })
        .catch((error) => {
            _self.tableData = []
