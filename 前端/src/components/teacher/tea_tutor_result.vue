<template>
  <div>
      <el-table :data="tableData" height="73vh" stripe style="width: 100%;" :row-style="rowState" v-on:row-click="rowClick" v-on:row-dblclick="rowdblclick" >
...
export default {
  ...
  methods: {
    ...
    rowClick:function(row, column, event){
      this.$emit('get-std', row.as_std_no)
    },
  ...
  beforeMount() {
    switch (this.userStatic.data_structure) {
      case 'teatutor'://導師查看學生學習成果及多元表現
        this.data_structure = data_structure.tea_tutor
        break;
      default:
    }
  },
