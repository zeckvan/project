<template>
  <div>
    ...
    <el-container>
      <el-main style="border: 1px solid #eee;width: 55%;">
        <el-table :data="tableData" 
        height=70vh 
        stripe 
        style="width: 100%" 
        :row-style="rowState" 
        v-on:row-click="leftRowClick"
        v-on:header-click="headerclick">
          ...
          <el-table-column align="center"  label="上傳檔案" width="" v-if="data_structure.upload_file">
            <template slot-scope="scope">
              <i class="el-icon-upload" style="cursor:pointer" @click="file_upload(scope)"></i>
            </template>
          </el-table-column>
          <el-table-column align="center" label="可上傳檔案數" width="" v-if="this.formSaveCheck == 'stuattestation'">
              <template slot-scope="scope">
                <el-badge :value="scope.row.x_file_cnt - scope.row.x_cnt2" class="item" type="primary"></el-badge>
              </template>
          </el-table-column>  
      ...
      <el-main style="border: 1px solid #eee;width: 45%;">
        <!--
        ...
        <el-table :data="rightData" height=70vh stripe style="width: 100%" :row-style="rowState" v-on:header-click="headerclick">
          <el-table-column type="index" width="50"></el-table-column>     
          <el-table-column prop="x_status"
                  width="80"
                  header-align="center"
          ...
          <el-table-column align="center" label="查看檔案" width="" v-if="data_structure.download_file">
              <template slot-scope="scope">
                <i class="el-icon-picture" style="cursor:pointer" @click="file_data(scope)"></i>                
              </template>
          </el-table-column>
          <el-table-column style="" align="center"  label="刪除資料" width="">
            <template slot-scope="scope">
...
export default {
  ...
  methods: {
    del_data:async function (val) {
      ...
      if(val.row.n == 'Y')
      {
        _self.$message.error('已完成選學習成果認證，無法刪除請確認!!');
        return false;
      }
    ...
    {
      this.src_dup = row.f
      this.rightData = this.tempData.filter(x => x.f == row.f && x.s != '')
    },
...
*/
          ...
          _self.check_list.forEach(function(value, index, array){
            ...
            formdata.append('complex_key[]',value.a +'_'+value.b+'_'+value.c+'_'+value.d+'_'+value.e+'_'+value.f+'_'+value.g+'_'+value.h+'@'+value.v)
          });
    ...
    file_upload: function (val) {
      ...
      if(_self.formSaveCheck == 'stuattestation') {
        if(val.row.x_file_cnt - val.row.x_cnt2 <= 0){
    ...
    getshow3: function (val) {    
      let _self = this
      let cnt = this.tableData[this.datarow.$index].x_cnt2
      ...
      if(val.success){
        ...
        this.tableData[this.datarow.$index].x_cnt2 = Number(cnt)
  ...
  beforeMount() {
    ...
    switch (this.userStatic.data_structure) {  
      ...
      case 'stuattestation'://學生諮詢課程查詢
        this.data_structure = data_structure.stu_attestation_course
