...
namespace StudentLearningHistory.Controllers
{
    ...
    [ApiController]
    public class StuLicenseController : Controller
    {
        private readonly StuLicenseService _service;
        ...
        public StuLicenseController(StuLicenseService service, IMapper mapper, IConfiguration configuration, ZipHelper zip, FileHelper file, GetUserProfilesService getUserProfilesService)
        {
            _service = service;
            _mapper = mapper;
        ...
        [HttpGet("list")]
        public async Task<IActionResult> GetList([FromQuery] StuLicenseQueryList arg)
        {
            ...
            StuLicenseQueryList cadre = _mapper.Map<StuLicenseQueryList>(arg);
        ...
        [HttpGet]
        public async Task<IActionResult> GetFormData([FromQuery] StuLicenseQueryList arg)
        {
            if (string.IsNullOrEmpty(arg.std_no))
            {
                TokenResult? token_data = _GetUserProfilesService.GetUserData(arg.token);
            ...
            StuLicenseQueryList cadre = _mapper.Map<StuLicenseQueryList>(arg);
            IEnumerable<L01_StuLicense> data = await _service.GetFormData(arg);
            return Json(new { dataset = data, status = (data.Count() <= 0 ? "N" : "Y") });
        }
        
        [HttpDelete]
        public async Task<IActionResult> DeleteData(StuLicenseQueryList arg)
        {
            TokenResult? token_data = _GetUserProfilesService.GetUserData(arg.token);
            arg.std_no = token_data.result.user_id;
            StuLicenseQueryList cadre = _mapper.Map<StuLicenseQueryList>(arg);
        ...
        [HttpPut]
        public async Task<IActionResult> UpdateData([FromForm] L01_StuLicense arg)
        {
            TokenResult? token_data = _GetUserProfilesService.GetUserData(arg.token);
            arg.std_no = token_data.result.user_id;
            L01_StuLicense cadre = _mapper.Map<L01_StuLicense>(arg);
        ...
        [HttpPost]
        public async Task<IActionResult> InsertData([FromForm] L01_StuLicense arg)
        {
            TokenResult? token_data = _GetUserProfilesService.GetUserData(arg.token);
            arg.std_no = token_data.result.user_id;
        ...
        [HttpPut("centraldb")]
        public async Task<IActionResult> UpdateDataCentraldb([FromForm] StuLicenseQueryList arg)
        {
            TokenResult? token_data = _GetUserProfilesService.GetUserData(arg.token);
            arg.std_no = token_data.result.user_id;
