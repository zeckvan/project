...
namespace StudentLearningHistory.Services
{
    public class StuResultService
    {
        ...
        public StuResultService(IDapperContext context, IConfiguration configuration)
        {
            _context = context;
        ...
        public async Task<IEnumerable<HeaderList>> GetList(StuResultQueryList arg)
        {
                                ...
                                SELECT *,
		                            ...
		                            x_total =
		                                     (select count(*)
                                                ...
                                                and   a.sch_no = NewTable.a 
                                                and   a.std_no = NewTable.d ),
                                '' as x_status
                                FROM(
                                        SELECT ROW_NUMBER() OVER(ORDER BY year_id, sms_id, result_date) AS RowNum,
                                                sch_no as a,
                                                year_id as b,
                                                sms_id as c,
                                                std_no as d,
        ...
        public async Task<IEnumerable<L01_StuResult>> GetFormData(StuResultQueryList arg)
        {
            arg.sch_no = _context.SchNo;
            string str_sql = @"
                                SELECT *
            ...
            using (IDbConnection conn = _context.CreateCommand())
            {
                return await conn.QueryAsync<L01_StuResult>(str_sql, arg);
            }
        }

        public async Task<int> DeleteData(StuResultQueryList arg)
        {
            arg.sch_no = _context.SchNo;
            string str_sql = @"
                                delete                                
        ...
        public async Task<int> UpdateData(L01_StuResult arg)
        {
            arg.sch_no = _context.SchNo;
        ...
        public async Task<Dictionary<string, string>> InsertData(L01_StuResult arg)
        {
            arg.sch_no = _context.SchNo;
            arg.upd_dt = updteDate();
            arg.upd_name = arg.std_no;
            ...
            using (IDbConnection conn = _context.CreateCommand())
            {
                ...
                var result = (from r in dt.AsEnumerable()
                              where r.Field<string>("result_name") == arg.result_name &&
                                    r.Field<string>("result_date") == arg.result_date
                              select r).Count();
                if (result > 0)
                {
                    dict.Add("haveData", "Y");
        ...
        public async Task<int> UpdateDataCentraldb(StuResultQueryList arg)
        {
